...
class Line {
    constructor(p1, p2) {
        this.p1 = p1;
        this.p2 = p2;
        this.length = p1.getMDistance(p2);
    }
    ...
    getDistanceFromInitialPoint(p) {
        if (this.isPointInLine(p)) {
            return this.p1.getMDistance(p);
        } else {
            return this.length;
        }
    }
...
function getDistanceInCircuit(circuit, point) {
    var distance = 0;
    for (line of circuit) {
        var d = line.getDistanceFromInitialPoint(point);
        if (d !== line.length) {
            return distance + d;
        }
        distance = distance + d;
...
var circuit2 = readLinesFromPoints(readPointsCircuit('R3,D3,L2,U6'));
 */
var intersectionPoints = getIntersectionPoints(circuit1, circuit2);

var minDistance = Number.MAX_VALUE;
for (iPoint of intersectionPoints) {
    var dist1 = getDistanceInCircuit(circuit1, iPoint);
    var dist2 = getDistanceInCircuit(circuit2, iPoint);
